...
test('submitTree - handles invalid body', t => {
  ...
  const submitJob = require('../../../lib/lambda-functions/trees/submit.js').post
  ...
  submitJob({ body: '{"not": "an array"}' }, {}, (err, result) => {
    ...
    t.equal(body.error, 'Missing required property: request', 'should complain about missing request')
    clearAllRequires()
    t.end()
  })
})
...
test.skip('submitTree - handle valid request without children', t => {
  ...
  const dbItems = [{ id: 234, treeId: 123, request: { url: 'http://a.com' }, requested: 12345 }]
  td.when(db.createAll('requests', dbItems, td.callback)).thenCallback(null, dbItems)
  ...
  const submitTree = require('../../../lib/lambda-functions/trees/submit.js').post
...
test.skip('submitTree - handle valid request with children', t => {
  ...
  const dbItems = [
    ...
    { id: 345, treeId: 123, parentId: 234, request: { url: 'http://b.com' }, requested: 12345 }
  ]
