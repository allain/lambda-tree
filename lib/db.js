...
function queryAll(tableName, conditions, fields, cb) {
  const filterExpression = Object.keys(conditions)
    .map(
      prop =>
        conditions[prop] !== null
          ? prop + ' =:' + prop
          : 'attribute_not_exists(' + prop + ')'
    )
    .join(' AND ')
  const projectionExpression = fields.join(',')
  const expressionAttributeValues = convert.toItem(
    Object.keys(conditions).reduce((result, prop) => {
      if (conditions[prop] !== null) {
...
function updateOne(tableName, key, props, cb) {
  const conditionExpression = Object.keys(key)
    .map(prop => prop + ' =:' + prop)
    .join(' AND ')
  const expressionAttributeValues = convert.toItem(
    Object.keys(key).reduce((result, prop) => {
