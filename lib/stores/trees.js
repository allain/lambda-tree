...
module.exports = {
  queryBranch: (treeId, requestId, cb) => {
    ...
    db.queryOne('requests', { treeId, id: requestId }, (err, request) => {
      ...
      db.queryAll('requests', { treeId, parentId: requestId }, ['id', 'product', 'productType'], (err, children) => {
  ...
  queryTree: (treeId, cb) => {
    assert(treeId, 'treeId not given')
    db.queryAll('requests', { treeId }, ['id', 'treeId', 'parentId', 'requested', 'started', 'finished', 'concurrency'], (err, requests) => {
      if (err) return cb(err)
  ...
  updateRequest(treeId, requestId, props, cb) {
    this.queryRequest(treeId, requestId, (err, request) => {
      ...
      db.updateOne('requests', { id: requestId, treeId }, Object.assign(request, props), cb)
    })
  }
}
